...
" Edited by manoj, enable syntax highlighting without checking for any
" condition
...
" use indents of 8 spaces, and have them copied down lines:
set shiftwidth=8
set shiftround
set autoindent
...
" Edited by manoj
" Treat .act files like cpp files
  autocmd BufNewFile,BufRead *.act set filetype=cpp
augroup END
...
" enables selecting the text into the system clipboard
set mouse=r

" sets the shell to be used when for :sh command
set shell=/usr/bin/bash

" highligt the search result
set hls
...
" map n and N to nzz and Nzz so that the search results are
" centered at the screen
nmap n nzz
nmap N Nzz

" put the tab number and file name in the tab header
" modified the function found at
" http://blog.golden-ratio.net/2008/08/19/using-tabs-in-vim/

" edited by manoj
" source the _pyvimrc for python coding standards

source C:\Program Files\Vim\_pyvimrc

" edited by manoj
" Autocompletion for python, js, html, css, xml and php and c
filetype plugin on
